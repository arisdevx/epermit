...
use App\Models\Area;
use App\Models\Convenience as ConvenienceModel;
...
class Convenience extends Controller
{
	public function index(Request $request)
	{
		...
		$data['conveniences'] = ConvenienceModel::where([
								'state_id' => $request->old('state'),
								'area_id' => $request->old('area'),
								'type' => $request->old('type'),
		...
		return view('tempahankemudahan.index', $data);
	}
	...
	public function store(ConvenienceRequest $request)
	{
		...
		{
			$getConvenience = ConvenienceModel::where([
		...
		if($getConvenience->capacity >= ($participant+$useCapacity))
		{
			...
			if($applicant->save())
			{
				...
				view()->share($data);
				$pdf = PDF::loadView('tempahankemudahan.download')->save(public_path('files/convenience/attachment_' . $applicant->number . '.pdf'));
				...
				// view()->share($data);
				// $pdf = PDF::loadView('aktivitilain.download')->save(public_path('files/otheractivity/attachment_' . $applicant->number . '.pdf'));
				...
				$redirect = redirect('tempahan-kemudahan/'. $applicant->id .'/view');
			}
			else
			{
	...
	public function edit($id)
	{
		...
		$data['subcategories'] = ConvenienceModel::where([
									'state_id' => $data['convenience_datax'][0]->state_id,
									'area_id' => $data['convenience_datax'][0]->area_id,
		...
		$data['types'] = ConvenienceModel::select('type')
						->where([
							'state_id' => $data['convenience_datax'][0]->state_id,
							'area_id' => $data['convenience_datax'][0]->area_id
						])
		...
		$data['categories'] = ConvenienceModel::where([
								  'state_id' => $data['convenience_datax'][0]->state_id,
								  'area_id' => $data['convenience_datax'][0]->area_id,
								  'type' => $data['convenience_datax'][0]->eco_park_type,
								  'eco_park_id' => $data['convenience_datax'][0]->eco_park_id
		...
		return view('tempahankemudahan.edit', $data);
	}
	...
	public function edit2($id)
	{
		...
		return view('tempahankemudahan.edit2', $data);
	}
	...
	public function update(ConvenienceRequest $request, $id)
	{
		...
		if($getConvenience->capacity >= ($participant+$useCapacity))
		{
			...
			{
				$redirect = redirect('tempahan-kemudahan/'. $id .'/edit')->with('status', 'failed');
			}
		}
		else
		{
			$redirect = redirect('tempahan-kemudahan/'. $id .'/edit')->with('status', 'failed-capacity');
		}
	...
	{
		$convenience = ConvenienceModel::where([
	...
	{
		$conveniences = ConvenienceModel::with(['convenience_category' => function($q){
							$q->orderBy('name', 'ASC');
						}])
						->where([
							'state_id' => $request->state_id,
	...
	{
		$conveniences = ConvenienceModel::select('type')
						->where([
							'state_id' => $request->state_id,
	...
	{
		$conveniences = ConvenienceModel::where([
							'state_id' => $request->state_id,
							'area_id' => $request->area_id,
							'type' => $request->type,
	...
	public function findConvenienceSubCategory(Request $request)
	{
		...
		{
			$convenience = ConvenienceModel::with(['convenience_sub_category' => function($q){
							$q->orderBy('name', 'ASC');
						}])
						->where([
							'state_id'                => $request->state_id,
	...
	public function review($id)
	{
		...
		$data['phd_data'] = RegionalForestry::where([
										...
										->first();
		return view('tempahankemudahan.review', $data);
	}
	...
	public function download($id)
	{
		...
		view()->share($data);
		$pdf = PDF::loadView('tempahankemudahan.download');

		return $pdf->download('Tempahan Kemudahan '. date('dmY') .'.pdf');

		// return view('tempahankemudahan.download', $data);
	}
	...
	public function save(ConvenienceRequest $request)
	{
		...
		return redirect('tempahan-kemudahan/'. $applicant->id .'/view');
