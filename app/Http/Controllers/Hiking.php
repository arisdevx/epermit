...
class Hiking extends Controller
{
	public function index(Request $request)
	{
		...
		return view('aktivitipendakian.index', $data);
	}
	...
	public function store(HikingRequest $request)
	{
		...
		if(($getMountain->capacity >= $participant))
		{
			...
			if($applicant->save())
			{
				...
				$applicantBiodata->user_id      = Auth::user()->id;
				...
				$applicantEmergency->user_id 		= Auth::user()->id;
				...
				$applicantInsurance->user_id 	  = Auth::user()->id;
				...
				$applicantDeclaration->user_id 	 = Auth::user()->id;
				...
				$applicantHealth->user_id 	   = Auth::user()->id;
				...
				return redirect('aktiviti-pendakian/view/' . $applicant->id)->with('status', 'success');
			}
			else
			{
				return redirect(url('aktiviti-pendakian'))->with('status', 'failed');
			}
		}
		else
		{
			return redirect(url('aktiviti-pendakian'))->with('status', 'failed-capacity');
		}
	}
	...
	public function edit($id)
	{
		...
		return view('aktivitipendakian.edit', $data);
	}
	...
	public function update(HikingRequest $request, $id)
	{
		...
		if($applicant->save())
		{
			...
			// 	}
			// }
			return redirect('aktiviti-pendakian/view/' . $id)->with('status', 'success');
			// return redirect(url('aktiviti-pendakian/'. $id .'/edit'))->with('status', 'success');
		}
		else
		{
			return redirect(url('aktiviti-pendakian/'. $id .'/edit'))->with('status', 'failed');
		}
	}
	...
	public function addUser($applicant_id)
	{
		...
		return view('aktivitipendakian.adduser', $data);
	}
	...
	public function addUserStore(ParticipantRequest $request, $id)
	{
		...
		$applicantParticipant               = new HikingParticipant;
		$applicantParticipant->user_id      = Auth::user()->id;
		...
		return redirect(url('aktiviti-pendakian/'. $id .'/'. Auth::user()->id .'/edit'));
	}
	...
	public function editUser($applicant_id, $user_id)
	{
		...
		return view('aktivitipendakian.edituser', $data);
	}
	...
	public function editUserUpdate(ParticipantRequest $request, $applicant_id, $user_id)
	{
		...
		return redirect(url('aktiviti-pendakian/'. $applicant_id .'/'. $user_id .'/edit'))->with('status', 'success');
	}
	...
	public function process($id)
	{
		...
		// return view('aktivitipendakian.document', $mail_data);

		view()->share($mail_data);
		$pdf = PDF::loadView('aktivitipendakian.document')->save(public_path('files/hiking/attachment_' . str_replace('/', '_', $applicant->number) . '.pdf'));
		$user = $applicant->user;
		$user['file'] = public_path('files/hiking/attachment_' . str_replace('/', '_', $applicant->number) . '.pdf');

		$email = Mail::send('aktivitipendakian.document', $mail_data , function ($mail) use ($user)
		{
			$mail->from(config('mail.from.address'), 'JPSM e-Permit');
			$mail->to($user->email, $user->name);
		...
		return redirect(url('application-status'))->with('status', 'success-processed');
	...
	public function view($id)
	{
		...
		$data['phd_data'] = RegionalForestry::where([
										...
										->first();
		return view('aktivitipendakian.review', $data);
	}
	...
	public function download($id)
	{
		...
		$pdf = PDF::loadView('aktivitipendakian.download');
