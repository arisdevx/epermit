...
@php($route = isset($category->id) ? route('price-category.update', $category->id) : route('price-category.store'))
@php($index = route('price-category.index'))

{!! Form::model($category, ['url' => $route, 'method' => $method, 'id' => 'price-category-form']) !!}
<div class="form-group" v-bind:class="errors.name ? 'has-error' : ''">
    <label class="control-label">
        <span v-if="errors.name">@{{ errors.name[0] }}</span>
        <span v-else>Jenis Harga</span>
    </label>
