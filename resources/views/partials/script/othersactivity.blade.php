	...
	(function($){
		...
		$(document).ready(function(){
			...
			$('body').on('change', '#state', function(){
				// $.ajax({
				// 	type: 'POST',
				// 	url: '{{ url('aktiviti-lain/find-eco-park') }}',
				// 	data: {
				// 		state_id: $(this).val(),
				// 		_token: '{{ csrf_token() }}'
				// 	},
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-lain/find-area') }}',
					data: {
						id: $(this).val(),
						_token: '{{ csrf_token() }}'
					},
			...
			$('body').on('change', '#place', function(){
				...
				if(category == 'hsk')
				{
					$.ajax({
						type: 'POST',
						url: '{{ url('aktiviti-lain/find-activity-price') }}',
						data: {
							id: $(this).val(),
							_token: '{{ csrf_token() }}',
							state: $('#state').val(),
			...
			$('body').on('change', '#area', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-lain/find-activity') }}',
					data: {
						state_id: $('#state').val(),
						area_id: $(this).val(),
						_token: '{{ csrf_token() }}'
			...
			$('body').on('change', '#starting_date', function(){
				...
				// $.ajax({
				// 	type: 'POST',
				// 	url: '{{ url('aktiviti-lain/find-capacity') }}',
				// 	data: {
			...
			$('body').on('change', '#category', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-lain/find-place') }}',
					data: {
						type: $(this).val(),
						state_id: $('#state').val(),
						area_id: $('#area').val(),
