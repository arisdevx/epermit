	...
	(function($){
		...
		$(document).ready(function(){
			...
			$('body').on('change', '#state', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-area') }}',
					data: {
						id: $(this).val(),
						_token: '{{ csrf_token() }}'
					},
			...
			$('body').on('change', '#area', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-mountain') }}',
					data: {
						state_id: stateId,
						area_id: areaId,
						// forest_id: forest_id,
				...
				// $.ajax({
				// 	type: 'POST',
				// 	url: '{{ url('aktiviti-pendakian/find-hsk') }}',
				// 	data: {
				// 		state_id: stateId,
				// 		area_id: areaId,
				// 		_token: '{{ csrf_token() }}'
			...
			$('body').on('change', '#type', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-eco-park') }}',
					data: {
						state_id: state_id,
						area_id: area_id,
						type: type,
			...
			$('body').on('change', '#mountain', function(){
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-amount-mountain') }}',
					data: {
						id: $(this).val(),
			...
			$('body').on('change', '#starting_date', function(){
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-ending-date') }}',
					data: {
			...
			$('body').on('change', '#participant', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-amount') }}',
					data: {
			...
			$('body').on('change', '#convenience', function(){
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-amount-convenience') }}',
					data: {
						id: $(this).val(),
						_token: '{{ csrf_token() }}'
					},
			...
			$('body').on('change', '#eco_park', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-convenience') }}',
					data: {
						state_id: state_id,
						area_id: area_id,
						type: type,
			...
			$('body').on('change', '#convenience_category', function(){
				...
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-convenience-sub-category') }}',
					data: {
						state_id: $('#state').val(),
						area_id: $('#area').val(),
			...
			$('body').on('change', '#category', function(){
				$.ajax({
					type: 'POST',
					url: '{{ url('aktiviti-pendakian/find-price-type') }}',
					data: {
						state_id: $('#state').val(),
						area_id: $('#area').val(),
						ecopark_id: $('#eco_park').val(),
			...
			$('body').on('change', '#checkName', function(){
				if($(this).is(':checked'))
				{
					@if(auth()->user()->profile->citizen == 1)
						$('#hiker_nationality').attr('checked', 'checked');
					@else
						$('#non_hiker_nationality').attr('checked', 'checked');
					@endif
					$('#hiker_state').val('{{ auth()->user()->profile->state }}').change();
					$('#hiker_fullname').attr('readonly', 'readonly');
					$('#hiker_fullname').val('{{ auth()->user()->name }}');
					$('#declaration_name').val('{{ auth()->user()->name }}');
					$('#declaration_ic').val('{{ (!empty(auth()->user()->profile->nokp) ? auth()->user()->profile->nokp : '') }}');
					var kp = '{{ (!empty(auth()->user()->profile->nokp) ? auth()->user()->profile->nokp : '') }}';
					if(kp != '')
					{
						var getDate = kp.substring(0, 6);
						var getArray = getDate.match(/.{1,2}/g);
					...
					@if(!empty(auth()->user()->profile->phone))
						$('#hiker_phone').val('{{ auth()->user()->profile->phonecode . auth()->user()->profile->phone }}');
					@endif

					@if(!empty(auth()->user()->profile->address_1))
					
						$('#hiker_address').val('{{ auth()->user()->profile->address_1 }}');
					@endif

					@if(!empty(auth()->user()->profile->postcode))
					
						$('#hiker_postcode').val('{{ auth()->user()->profile->postcode }}');
					@endif

					@if(!empty(auth()->user()->profile->state))
					
						$('#hiker_state').val('{{ auth()->user()->profile->state }}').trigger('change');
					@endif

					@if(!empty(auth()->user()->profile->country))
						$('#hiker_country').val('{{ auth()->user()->profile->country }}').trigger('change');
					@endif
			...
			$('#checkAddress').on('click', function(){
				if($(this).is(':checked'))
				{
					$('#emergency_address').val('{{ auth()->user()->profile->address_1 }}');
				}
				else
				{
