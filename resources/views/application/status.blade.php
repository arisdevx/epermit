											...
											<table class="table table-bordered" width="100%" cellspacing="10px" cellpadding="0"> 
												...
												@foreach($status->hikingParticipant as $participant)
													<tr style="background-color: #ffffff !important">
														...
														<td style="padding: 5px !important"><a href="{{ url('aktiviti-pendakian/' . $status->id . '/' . $participant->id . '/edit') }}">{{ $participant->hikingBiodata->fullname }}</a> {!! (in_array($participant->id, $status->hikingGuide()->pluck('participant_id')->toArray()) ? '(Malim / <i>Guide</i>)' : '') !!}</td>
													</tr>
												@endforeach
											</table>
										@endif
								...
								<td align="top" style="vertical-align: top">
										...
										@if($status->user_id == auth()->user()->id)
											@if($status->status == 'new')
												...
												@if($status->hikingInformation->participants_total <= ($status->hikingParticipant->count()-1))
													<a href="{{ url('aktiviti-pendakian/process/' . $status->id) }}" class="btn btn-success btn-sm btn-block">Hantar / <i>Send</i></a> 
												@else
													<a href="#" 
													   class="btn btn-default btn-sm btn-block"
													   data-toggle="modal"
					                                   data-target="#modal-notification"
					                                   {{-- data-action="{{ route('application-status.destroy', $status->id) }}" --}}
					                                   data-title="Maklumat"
				                                ...
				                                <a href="{{ route('aktiviti-pendakian.edit', $status->id) }}" class="btn btn-primary btn-sm btn-block">Kemaskini / <i>Edit</i></a>
											@endif
											@if($status->status == 'processed')
				                                ...
				                                <a href="{{ route('aktiviti-pendakian.edit', $status->id) }}" class="btn btn-primary btn-sm btn-block">Kemaskini / <i>Edit</i></a>
											@elseif($status->status == 'canceled')
												<a href="{{ url('application-status/'. $status->id .'/hiking') }}" class="btn btn-default btn-sm btn-block"><i class="material-icons">remove_red_eye</i></a>
									...
									@elseif($status->type == 'convenience')
										...
										@if($status->status == 'processed')
			                                ...
			                                <a href="{{ route('tempahan-kemudahan.edit', $status->id) }}" class="btn btn-primary btn-sm btn-block">Kemaskini / <i>Edit</i></a>
			                            @elseif($status->status == 'canceled')
			                            	<a href="{{ url('application-status/'. $status->id .'/convenience') }}" class="btn btn-default btn-sm btn-block"><i class="material-icons">remove_red_eye</i></a>
			                            	{{-- <button class="btn btn-danger btn-sm btn-block" disabled>Batal</button>  --}}
			                            @elseif($status->status == 'finished')
			                            	{{-- <button class="btn btn-danger btn-sm btn-block" disabled>Selesai</button> --}}
			                            	@if(!empty($status->receipt_number))
		                            ...
		                            @elseif($status->type == 'other')
			                            ...
			                            @elseif($status->status == 'canceled')
			                            	<a href="{{ url('application-status/'. $status->id .'/other') }}" class="btn btn-default btn-sm btn-block"><i class="material-icons">remove_red_eye</i></a>
			                            	{{-- <button class="btn btn-danger btn-sm btn-block" disabled>Batal</button>  --}}
			                            @elseif($status->status == 'finished')
			                            	{{-- <button class="btn btn-danger btn-sm btn-block" disabled>Selesai</button> --}}
			                            	@if(!empty($status->receipt_number))
			                            ...
			                            @elseif($status->status == 'completed')
			                            	<button class="btn btn-primary btn-sm btn-block" disabled>Kemaskini / Edit</button>
			                            @endif
		                            	{!! ($status->status == 'processed' ? '<a href="'. route('aktiviti-lain.edit', $status->id) .'" class="btn btn-primary btn-sm btn-block">Kemaskini / <i>Edit</i></a>' : '') !!}
									@endif
								</td>
							</tr>
						@endforeach
								...
								<td>
									...
									@if($status->type == 'hiking')
										...
										@else
											...
											@foreach($status->hikingParticipant as $participant)
												{{ $hikers++ }}. 
												<a href="{{ url('aktiviti-pendakian/' . $status->id . '/' . $participant->id . '/edit') }}">{{ $participant->hikingBiodata->fullname }}</a>
												<br />
											@endforeach
								...
								<td>
										...
										@if($status->user_id == auth()->user()->id)
											@if($status->status == 'new')
												...
												@if($status->hikingInformation->participants_total == $status->hikingParticipant->count())
													<a href="{{ url('aktiviti-pendakian/process/' . $status->id) }}" class="btn btn-success btn-sm btn-block">Hantar</a> 
												@else
													<a href="#" 
													   class="btn btn-default btn-sm btn-block"
													   data-toggle="modal"
				                                ...
				                                <a href="{{ route('aktiviti-pendakian.edit', $status->id) }}" class="btn btn-primary btn-sm btn-block">Kemaskini</a>
											@endif
											@if($status->status == 'processed')
												<a href="javascript:void(0)"
												   class="btn btn-danger btn-sm btn-block"
									...
									@elseif($status->type == 'convenience')
										...
										@if($status->status == 'processed')
			                                ...
			                                <a href="{{ route('tempahan-kemudahan.edit', $status->id) }}" class="btn btn-primary btn-sm btn-block">Kemaskini</a>
			                            @elseif($status->status == 'canceled')
			                            	<a href="{{ url('application-status/'. $status->id .'/convenience') }}" class="btn btn-default btn-sm btn-block"><i class="material-icons">remove_red_eye</i></a>
			                            	{{-- <button class="btn btn-danger btn-sm btn-block" disabled>Batal</button>  --}}
			                            @elseif($status->status == 'finished')
		                            ...
		                            @elseif($status->type == 'other')
			                            ...
			                            @elseif($status->status == 'completed')
			                            	<button class="btn btn-primary btn-sm btn-block" disabled>Kemaskini</button>
			                            @endif
		                            	{!! ($status->status == 'processed' ? '<a href="'. route('aktiviti-lain.edit', $status->id) .'" class="btn btn-primary btn-sm btn-block">Kemaskini</a>' : '') !!}
									@endif
								</td>
							</tr>
						@endforeach
